...
// Count total records
$countSql = "SELECT COUNT(*) FROM scholarships WHERE level = 'phd'";
if ($search) {
...
$sql = "SELECT * FROM scholarships WHERE level = 'phd'";
if ($search) {
...
<div class="container mt-4">
    <h2 class="text-danger">PhD Scholarships</h2>
    ...
    <?php while ($row = $result->fetch_assoc()): ?>
        <div class="card mb-3 shadow-sm">
            <div class="card-body">
                ...
                <p><?= $row['description'] ?></p>
                <a href="<?= $row['link'] ?>" target="_blank" class="btn btn-sm btn-danger">Apply Now</a>
            </div>
        </div>
    <?php endwhile; ?>
